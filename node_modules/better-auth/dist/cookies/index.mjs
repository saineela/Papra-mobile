...
import { parseCompactCookieCache, parseCookieCachePayload } from "./cache.mjs";
...
import { verifySessionCookieJwtWithJwks } from "./jwt.mjs";
...
const getCookieCache = async (request, config) => {
	...
	if (sessionData) {
		...
		if (strategy === "jwe") {
			...
			const payload = parseCookieCachePayload(await symmetricDecodeJWT(sessionData, secret, "better-auth-session"));
			if (payload) {
				if (config?.version) {
			...
			let payload;
			if (jwks) payload = await verifySessionCookieJwtWithJwks(sessionData, jwks, {
				...
				const secret = config?.secret || env.BETTER_AUTH_SECRET;
				if (!secret) throw new BetterAuthError("getCookieCache requires a secret to be provided. Either pass it as an option or set the BETTER_AUTH_SECRET environment variable");
				payload = parseCookieCachePayload(await verifyJWT(sessionData, secret));
			}
			if (payload) {
				if (config?.version) {
					const cookieVersion = payload.version || "1";
			...
			const secret = config?.secret || env.BETTER_AUTH_SECRET;
			if (!secret) throw new BetterAuthError("getCookieCache requires a secret to be provided. Either pass it as an option or set the BETTER_AUTH_SECRET environment variable");
			const sessionDataPayload = parseCompactCookieCache(safeJSONParse(binary.decode(base64Url.decode(sessionData))));
...
//#endregion
export { HOST_COOKIE_PREFIX, SECURE_COOKIE_PREFIX, applySetCookies, cookieNameRegex, createCookieGetter, createSessionStore, decodeCookieCache, deleteSessionCookie, expireCookie, getAccountCookie, getChunkedCookie, getCookieCache, getCookies, getSessionCookie, parseCookies, parseSetCookieHeader, setAccountCookie, setCookieCache, setCookieToHeader, setRequestCookie, setSessionCookie, splitSetCookieHeader, stripSecureCookiePrefix, toCookieOptions };
